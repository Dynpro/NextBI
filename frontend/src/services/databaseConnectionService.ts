...
export const databaseConnectionService = {
  ...
  // Execute SQL query
  async executeQuery(connectionId: string, payload: { query?: string; params?: any }): Promise<any[]> {
    // Validate payload early
    if (!payload || typeof payload.query !== 'string' || !payload.query.trim()) {
      // Throw a clear client-side error instead of making a bad backend request
      throw new Error('Invalid query: query text is empty. Please configure the tile with a dimension and measure query.');
    }

    try {
      const authHeaders = await getAuthHeaders();
      const response = await axios.post(`${API_URL}/${connectionId}/query`, { query: payload.query, params: payload.params }, { headers: authHeaders });
      return response.data.data;
    } catch (error) {
