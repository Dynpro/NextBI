...
const DataModels: React.FC = () => {
  ...
  const fetchDatabaseSchema = async (connectionId: string) => {
    try {
      ...
      // Get database schema using service (may return only table names or simple columns)
      const schemaData = await dataModelService.getDatabaseSchema(connectionId);

      // Helper to extract rows robustly from various service response shapes
      const extractRows = (res: any): any[] => {
        ...
        // If it's an object of tables -> columns like { tables: { t1: [...], ... } }
        return [];
      };

      // Normalize schema into expected format
      let schema: DatabaseSchema = { tables: [] };

      // If backend provided structured tables object
      if (schemaData && schemaData.tables && typeof schemaData.tables === 'object') {
        const tableEntries = Object.entries(schemaData.tables);
        // If the connection is Postgres, try to enrich column metadata via information_schema
        // Find the connection type to choose metadata query
        const connection = await databaseConnectionService.getConnectionById(connectionId).catch(() => null);

        const isPostgres = connection?.type === 'postgresql';

        // Cache per-table metadata to avoid duplicate queries
        const tableMetaCache: Record<string, SchemaTable> = {};

        for (const [tableName, cols] of tableEntries) {
          // If the service already returned detailed column objects, use them
          if (Array.isArray(cols) && cols.length > 0 && typeof cols[0] === 'object' && cols[0].name) {
            schema.tables.push({
              name: tableName,
              columns: (cols as any[]).map(c => ({
                name: c.name,
                dataType: c.dataType || c.type || 'unknown',
                nullable: c.nullable === undefined ? true : !!c.nullable,
                maxLength: c.maxLength || c.character_maximum_length || undefined
              })),
              primaryKey: (schemaData.primaryKeys && schemaData.primaryKeys[tableName]) || undefined,
              foreignKeys: (schemaData.foreignKeys && schemaData.foreignKeys[tableName]) || undefined
            });
            continue;
          }

          // If columns are strings (names) and Postgres, query information_schema
          if (Array.isArray(cols) && cols.length > 0 && typeof cols[0] === 'string' && isPostgres) {
            try {
              if (!tableMetaCache[tableName]) {
                const infoQuery = `
                  SELECT
                    column_name,
                    data_type,
                  ...
                  WHERE table_name = ${databaseConnectionService.escapeValue(tableName)}
                  ORDER BY ordinal_position;
                `;
                const infoResp = await databaseConnectionService.executeQuery(connectionId, { query: infoQuery });
                const rows = extractRows(infoResp);
                const columns = rows.map((r: any) => ({
                  name: r.column_name || r.name,
                  dataType: r.data_type || r.dataType || 'unknown',
                  nullable: (r.is_nullable || '').toLowerCase() === 'yes' || r.nullable === true,
                  maxLength: r.character_maximum_length || r.maxLength || undefined
                }));
                tableMetaCache[tableName] = { name: tableName, columns };
              }
              schema.tables.push(tableMetaCache[tableName]);
            } catch (err) {
              console.warn(`Failed to enrich schema for table ${tableName}:`, err);
              // Fallback: use column names only
              schema.tables.push({
                name: tableName,
                columns: (cols as string[]).map(c => ({
                  name: c,
                  dataType: 'unknown',
                  nullable: true
                }))
          ...
          // Fallback: generic handling when cols is array of strings or missing
          if (Array.isArray(cols)) {
            schema.tables.push({
              name: tableName,
              columns: (cols as string[]).map(c => ({
                name: typeof c === 'string' ? c : (c && c.name) || 'unknown',
                dataType: (c && (c.dataType || c.type)) || 'unknown',
                nullable: c && c.nullable !== undefined ? !!c.nullable : true
              }))
            });
            continue;
          }

          // If nothing matched, add an empty table entry
          schema.tables.push({
            name: tableName,
            columns: []
          });
        }
      } else if (Array.isArray(schemaData) && schemaData.length > 0) {
        // If backend returned array of table names or rows, try to interpret as table list
        // Here we treat each entry as a table name string
        for (const t of schemaData) {
        ...
        // Unknown schema shape: return empty
        schema = { tables: [] };
      }
